...
void	render_walls(t_data *data)
{
	...
	while (column_id < data->conf.num_rays)
	{
		...
		* cos(data->rays[column_id]->angel - data->player.rotation_angel);
		data->conf.projected_wall_heigth = (TILE_SIZE *
		distance_from_player_to_projection)
		/ distance_from_player_to_wall;
		scale = distance_from_player_to_projection /
		distance_from_player_to_wall;
...
void	texture_walls(t_data *data, t_ray *ray, int column_id, double scale)
{
	...
	while (tex_y < tex.height)
	{
		...
		(data->conf.win_h / 2 - (int)data->conf.projected_wall_heigth / 2))
		* data->img.line_length + column_id * (data->img.bits_per_pixel / 8));
		a = tex.img_addr + tex_y *
