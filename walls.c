...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/11/02 18:44:53 by fflores           #+#    #+#             */
/*   Updated: 2020/11/02 18:44:54 by fflores          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_walls(t_data *data)
{
	...
	{
		data->conf.rm_wall_heigth = 0;
		distance_from_player_to_wall = (data->rays[column_id]->distance)
		* cos(fabsf(data->rays[column_id]->angel
		- data->player.rotation_angel));
		data->conf.projected_wall_heigth = (TILE_SIZE * dpp)
		/ distance_from_player_to_wall;
		if (data->conf.projected_wall_heigth > data->conf.win_h)
			data->conf.rm_wall_heigth = data->conf.projected_wall_heigth
			- data->conf.win_h;
...
void	texture_walls(t_data *data, t_ray *ray, int column_id, double scale)
{
	...
	while (tex_y < tex.height && tex_y < data->conf.win_h)
	{
		c = data->img.img_addr + ((tex_y +
		(data->conf.win_h / 2 - (int)data->conf.projected_wall_heigth
		/ 2 + (int)data->conf.rm_wall_heigth / 2))
		* data->img.line_length + column_id * (data->img.bits_per_pixel / 8));
		a = tex.img_addr + (tex_y + (int)data->conf.rm_wall_heigth / 2) *
